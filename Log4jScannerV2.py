import requests
from InteractshProcess import interactsh
...
def updateConfig():
    ...
    configList = userInteract.argumentHandling(requestFilePath, isGetMethod, headerDict, reqBody, url, query, interact_url, thread, HTTP_PROXY, excludedHeader, outputFile, verboseInf, isOOBVector, injectAllHeader, defaultHeader, payloads, mode, notAffectedHeader)
    # len(configList) == 18 xảy ra khi người dùng không sử dụng option cung cấp request bằng file (-u thay vì -f)
    if len(configList) == 18:
        requestFilePath, isGetMethod, headerDict, reqBody, url, query, interact_url, thread, HTTP_PROXY, excludedHeader, outputFile, verboseInf, isOOBVector, \
...
def scanFlow():
    ...
    if requestFilePath:
        ...
        for request in requestList:
            ...
            scanTool = scanner(mode, modeList, payloads, util, reqBody, defaultHeader, query, headerDict,
                               injectAllHeader, excludedHeader)
